...
状态侧天然轻:只需保存**开着的窗口**(已见 A 未见 C 的 `aPos` 条目)和**未分配的 B**——`Cᵢ` 一结算就 `aPos.remove(i)`、B 一分配就删除(批量版懒得写的 remove,流式里成为义务)。内存 = **峰值并发窗口数**,不是日志总量;兜底不变量:每条未分配 B 必被某个还开着的窗口认领,故未分配 B 数 ≤ 开窗数 × k。

输出侧要诚实指出缓冲需求:B 的 id 要等未来的 C 结算才揭晓,每条未分配 B 都是输出流里的"**空洞**"——**只能 flush 到最早空洞之前**,其后 token(含已是最终形态的 A/C)先缓冲,结算填洞后 flush 新连续前缀——与 **TCP 重排缓冲**同构,点出这个同构值一句台词。

下界的精确说法:当**两个开窗同时罩住一条 B** 且双方的 C 都很晚才来,这条 B 归谁在信息上就是未定的——任何算法都得等,缓冲被拖住是**问题本身的下界**,不是实现缺陷。(可讲的小优化:只被**一个**开窗罩住的 B 是唯一候选,可当场定案提前放行;真正拖住输出的必然是 ≥2 候选的 B。)
