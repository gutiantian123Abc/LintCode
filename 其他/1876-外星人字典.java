/**
 * 1876 · 外星人字典
 * https://www.lintcode.com/problem/1876/
 * https://leetcode.com/problems/verifying-an-alien-dictionary/
 *
 * 描述
 * 某种外星语也使用英文小写字母，但可能顺序 order 不同。
 * 字母表的顺序（order）是一些小写字母的排列。
 * 给定一组用外星语书写的单词 words，以及其字母表的顺序 order，
 * 只有当给定的单词在这种外星语中按字典序排列时，返回 true；否则，返回 false。
 *
 * 样例1：
 * 输入：
 * words = ["hello","leetcode"], order = "hlabcdefgijkmnopqrstuvwxyz"
 * 输出：
 * true
 * 解释：
 * 在该语言的字母表中，'h' 位于 'l' 之前，所以单词序列是按字典序排列的。
 *
 * 样例2:
 * 输入：
 * words = ["word","world","row"], order = "worldabcefghijkmnpqstuvxyz"
 * 输出：
 * false
 * 解释：
 * 在该语言的字母表中，'d' 位于 'l' 之后，那么 words[0] > words[1]，因此单词序列不是按字典序排列的。
 *
 * 样例3:
 * 输入：
 * words = ["apple","app"], order = "abcdefghijklmnopqrstuvwxyz"
 * 输出：
 * false
 * 解释：
 * 当前三个字符 "app" 匹配时，第二个字符串相对短一些，
 * 然后根据词典编纂规则 "apple" > "app"，因为 'l' > '∅'，其中 '∅' 是空白字符，定义为比任何其他字符都小（更多信息）。
 */
public class Solution {
    /**
     * @param words: the array of string means the list of words
     * @param order: a string indicate the order of letters
     * @return: return true or false
     */
    public boolean isAlienSorted(String[] words, String order) {
        // write your code here.
        // 记住， 字典顺序you传递性：
        // if wordA < wordB, wordB < wordC, then => wordA < wordC
        Map<Character, Integer> map = new HashMap<>();
        for (int i = 0; i < order.length(); i++) {
            map.put(order.charAt(i), i);
        }
        for (int i = 1; i < words.length; i++) {
            String wordA = words[i - 1];
            String wordB = words[i];
            if (!compare(wordA, wordB, map)) {
                return false;
            }
        }

        return true;
    }

    private boolean compare(String A, String B, Map<Character, Integer> map) {
        int minLen = Math.min(A.length(), B.length());
        int i = 0;
        while (i < minLen) {
            char a = A.charAt(i);
            char b = B.charAt(i);

            int aIndex = map.get(a);
            int bIndex = map.get(b);

            if (aIndex < bIndex) {
                // WordA is lexicographically smaller than WordB
                // 后面就不用比了
                return true;
            } else if (aIndex > bIndex) {
                // WordA is lexicographically larger than WordB
                return false;
            }
            i++;
        }

        if (A.length() > B.length()) {
            return false;
        }

        return true;
    }
}